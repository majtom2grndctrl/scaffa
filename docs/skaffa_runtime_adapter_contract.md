# Skaffa Runtime Adapter Contract (v0)
...
- Also load: `docs/skaffa_runtime_adapter_integration_guide.md`, `docs/skaffa_preview_session_protocol.md`, `docs/skaffa_override_model.md`.

Related:
- [Architecture Plan](./index.md)
- [Skaffa Runtime Adapter Integration Guide](./skaffa_runtime_adapter_integration_guide.md)
...
A **Runtime Adapter** is the framework-specific code that runs *inside the preview runtime* (the app preview or component preview) to provide Skaffa with:
...
- Projects SHOULD be able to build and run in production without Skaffa installed or enabled.
- Runtime adapter integration is intended to be dev-only (or Skaffa-only) instrumentation of the preview runtime.
- Recommended v0 integration path is the Harness Model (managed launcher + virtual harness entry), so projects do not need Skaffa editor/runtime adapter imports in production code.
...
- adapter must follow Skaffa’s interaction policy for the session (Editor View vs Preview Mode)
...
export type PreviewSessionId = string; // opaque, generated by Skaffa host
...
Runtime adapters communicate with Skaffa via a message transport owned by the **Preview Session** (see `docs/skaffa_preview_session_protocol.md`). The contract below describes minimum intent; the exact transport may be `postMessage`, `MessagePort`, or an Electron bridge owned by the session.
...
1. **Instance identity**  
   - Assign a stable `instanceId` during render using a small wrapper component (e.g. `SkaffaInstanceBoundary`) or React instrumentation.
   - Optionally annotate a DOM node with `data-skaffa-instance-id` for hit-testing selection.

2. **Click-to-select**  
   - Capture pointer events at the document root.
   - Walk up the DOM to find the nearest `data-skaffa-instance-id`.
...
- Skaffa-owned overlay layer (eliminates most CSS conflicts)
...
- Arbitrary DOM mutation from Skaffa
