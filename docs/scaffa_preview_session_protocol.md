...
export type PreviewSessionTarget =
  | {
      type: "app";
      /**
       * Attached mode: Scaffa attaches to an already-running dev server.
       * Exactly one of `url` or `launcherId` MUST be provided.
       */
      url?: string;
      /**
       * Managed mode (via a launcher module): Scaffa starts/manages the dev server.
       * Exactly one of `url` or `launcherId` MUST be provided.
       */
      launcherId?: string;
...
Rationale: `BrowserView` keeps the renderer sandboxed (no `webviewTag`) while allowing Scaffa to manage the preview `WebContents` as a host-owned capability.
