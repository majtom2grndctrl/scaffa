        ...
        if (eventType === 'change') {
          ...
          try {
            ...
            const ops: GraphPatch['ops'] = [
              ...
              ...nodes.map((node) => ({ op: 'upsertNode' as const, node })),
              ...edges.map((edge) => ({ op: 'upsertEdge' as const, edge })),
            ];
 ...
 */
async function parseRouteModule(filePath: string): Promise<RouteDefinition[]> {
 ...
 */
function parseRouteObject(node: ts.ObjectLiteralExpression): RouteDefinition | null {
  ...
  // Require both id and path for v0
  if (!id || !path) {
    console.warn(
