...
function resolveUiRoot(workspaceRoot) {
  if (!workspaceRoot) {
    return 'src/components/ui';
  }

  const appUiPath = path.join(workspaceRoot, 'app', 'src', 'components', 'ui');
  if (fs.existsSync(appUiPath)) {
    return 'app/src/components/ui';
  }
...
export function activate(context) {
  ...
  const uiRoot = resolveUiRoot(context.workspaceRoot);
