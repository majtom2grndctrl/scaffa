...
`;
    ...
    if (namedFuncMatch) {
      ...
      // Replace named export with wrapped version
      const wrappedExport = `
import { ScaffaInstanceBoundary as _ScaffaInstanceBoundary } from '@scaffa/react-runtime-adapter';

${code.replace(namedFuncPattern, `const _Original${exportName} = ${keyword}`)}

export const ${exportName} = _ScaffaInstanceBoundary(_Original${exportName}, ${JSON.stringify(typeId)});
`;
      return wrappedExport;
    }
...
`;
    ...
    const harnessPlugin = {
      ...
      transformIndexHtml(html) {
        const scriptRegex = /<script\s+type="module"\s+src="[^"]+">/;
        const match = html.match(scriptRegex);
        console.log("[ViteRunner] transformIndexHtml replacing:", match ? match[0] : "NO MATCH");
        return html.replace(
          scriptRegex,
          '<script type="module" src="/.scaffa-harness.tsx">'
        );
      }
    };
