...
export const InspectorPanel = () => {
  ...
  const isRegistryLoading = useInspectorStore(
    (state) => state.isRegistryLoading,
  );
  const inspectorSections = useInspectorStore(
    (state) => state.inspectorSections,
  );
  const isSectionsLoading = useInspectorStore(
    (state) => state.isSectionsLoading,
  );
  ...
  useEffect(() => {
    ...
    console.warn(
      `[Inspector]   • ComponentTypeId: ${selectedInstance.componentTypeId}`,
    );
    console.warn(`[Inspector]   • Instance ID: ${selectedInstance.instanceId}`);
    ...
    console.warn(
      '[Inspector]     1. Registry entry key (e.g., components["ui.button"])',
    );
  ...
  const handleClearOrphanedOverrides = async () => {
    ...
    try {
      ...
      console.log(
        "[Inspector] Cleared orphaned overrides:",
          ...
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-medium text-warning">
                {orphanedOverrides.length} orphaned override
                {orphanedOverrides.length !== 1 ? "s" : ""}
              </p>
            ...
            {instanceOverrideCount > 0 ? (
              <>
                <span className="inline-flex items-center bg-selected px-2 py-0.5 text-xs font-medium text-selected-fg">
                  {instanceOverrideCount} override
                  {instanceOverrideCount !== 1 ? "s" : ""}
                </span>
              </>
            ) : (
        ...
        {!isRegistryLoading && !registryEntry && (
          <div className="border border-warning bg-warning-subtle px-3 py-2">
            ...
            <p className="mt-2 text-xs text-fg-muted">
              Props cannot be edited without a registry entry. The Inspector
              will show instance metadata only.
            </p>
            ...
            {inspectorSections.map((section) => {
              const context: InspectorSectionContext = {
                ...
                overrides: overrides.filter(
                  (override) =>
                    override.instanceId === selectedInstance.instanceId,
                ),
              };

              return (
                <div key={section.id} className="border-t border-subtle pt-4">
                  <ExtensionSection section={section} context={context} />
                </div>
              );
            })}
          </div>
...
const PropField = ({
  ...
  const hasRuntimeBaseline =
    runtimeProps !== undefined && propName in (runtimeProps ?? {});
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (hasRuntimeBaseline) {
        ...
        exposure.uiDefaultValue !== undefined
      ) {
      ...
      console.log("[PropField] Override cleared:", {
        propName,
        resetTo: baselineValue,
      });
    } catch (error) {
