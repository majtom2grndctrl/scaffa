...
describe("Selection Flow - Renderer Integration (End-to-End)", () => {
  ...
  beforeEach(async () => {
    ...
    // Set up window.skaffa mock without clobbering jsdom window
    (globalThis.window as any).skaffa = {
  ...
  it("should update Inspector when user selects different element", async () => {
    ...
    expect(useInspectorStore.getState().selectedInstance?.displayName).toBe(
      "First Button",
    );
  ...
  it("should preserve session context in selection", async () => {
    ...
    expect(useInspectorStore.getState().selectedInstance?.sessionId).toBe(
      sessionId,
    );
  });
  ...
  it("should preserve complete instance descriptor including props and locator", async () => {
    ...
    expect(selected?.instanceLocator).toEqual({
      kind: "renderIndex",
      index: 3,
    });
